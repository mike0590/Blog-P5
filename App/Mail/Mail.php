...
class Mail
{
	...
	public static function sendRestartMail($id, $mail, $key, $username)
	{
		$destinataire = $mail; // Déclaration de l'adresse de destination.
			...
			if (isset($inscription)) {
				$message_html = '<div style="width: 100%; font-weight: bold">Bonjour, 
							cliquez sur le lien si-dessous pour vous inscrire:<br/>
							<a href="http://www.passion-php.fr/inscription/token/' .$key. '">Inscription</a></div>';
			} else{
	...
	public static function sendInscriptionMail($id, $mail, $key, $username)
	{
		$destinataire = $mail; // Déclaration de l'adresse de destination.
			if (!preg_match("#^[a-z0-9._-]+@(hotmail|live|msn).[a-z]{2,4}$#", $destinataire)) // On filtre les serveurs qui rencontrent des bogues.
			{
